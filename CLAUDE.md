...
- **Notifications / activity:** SECURITY DEFINER helpers `app_notify`, `app_notify_manco`,
  ...
  `postgres_changes` and debounce-`ping()`s a reload; **21 tables** in the `supabase_realtime` publication). See §12 Batch C.
...
**Batch C — status:**
- DONE: **Realtime** live sync (migration 0012 — **21 tables** in supabase_realtime as of the latest loads; repo.ts subscribes to postgres_changes and debounce-pings the reload). Clients auto-refresh, no manual refresh.
