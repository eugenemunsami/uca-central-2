-- 0013_discovery_gate.sql
-- Discovery links: Google-Form links embedded in the SOW, one per intervention where applicable, that
-- give the consultant their brief. After acknowledging a new intervention the consultant confirms
-- whether the beneficiary has filled that form (Complete / Incomplete / Not applicable). Crucially,
-- an intervention's SLA/RAG timers only start once the discovery phase is CLEARED — until then the
-- intervention sits green ("Awaiting discovery form").
--
-- discovery_status values: 'pending' (default, new work), 'cleared' (form filled -> start work),
-- 'incomplete' (form not filled -> follow up or escalate), 'na' (not applicable -> start work).
-- 'cleared' and 'na' both mean the phase is done and timers run.

alter table interventions add column if not exists discovery_status text default 'pending';
alter table interventions add column if not exists discovery_link text;
alter table interventions add column if not exists discovery_at timestamptz;

-- Existing interventions are already in flight — treat their discovery phase as done.
update interventions set discovery_status = 'na' where discovery_status is null or discovery_status = 'pending';

-- Expose the discovery columns on the RAG view and pause the timers until discovery clears.
create or replace view v_intervention_rag with (security_invoker=on) as
 SELECT i.id, i.beneficiary_id, i.kind, i.catalogue_id, i.custom_name, i.custom_kind, i.custom_budget,
    ...
    CASE
        ...
        WHEN i.discovery_status IS NOT NULL AND i.discovery_status NOT IN ('cleared','na') THEN 'green'::rag
    ...
    CASE
        ...
        WHEN i.discovery_status IS NOT NULL AND i.discovery_status NOT IN ('cleared','na') THEN 'Awaiting discovery form'::text
        ...
        ELSE NULL::text
    END AS rag_reason,
    i.discovery_status, i.discovery_link, i.discovery_at
